...
// resolver from `<ViskoOrbisStableProvider jwtToken={…} />` via React context, so
...
// `app/layout.tsx`) — capture the resolver with
// `reactor.getJwtResolver()` at action time and thread it down.
//
// `requestClip` / `requestRecording` / `downloadClipAsFile` sit directly
// on the React store, so the recording surface is reachable via
// `useReactor((s) => s.requestClip)`. The clip components also accept
// `onError` / `onSuccess` callbacks — this panel routes both player and
// download failures into its inline error line, and clears that line
